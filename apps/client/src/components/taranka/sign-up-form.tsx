...
export function SignUpForm() {
    ...
    return (
      <div className="w-full max-w-[420px] rounded-[20px] bg-white p-8 font-taranka-body sm:p-10">
        ...
        <p className="mt-2 text-sm text-ink-900/70">
          {t("verify.sentTo")} <span className="font-medium">{email}</span>.
        </p>
  ...
  return (
    <div className="w-full max-w-[420px] rounded-[20px] bg-white p-8 font-taranka-body sm:p-10">
      ...
      <p className="mt-5 text-sm font-normal text-ink-900">{t("signUp.accountTypePrompt")}</p>
      <div className="mt-2 grid grid-cols-2 gap-1 rounded-full bg-[#EDEBE3] p-1">
        {(
          [
            { value: "RETAIL", label: t("signUp.retail") },
            { value: "WHOLESALE", label: t("signUp.wholesale") },
          ] as const
        ).map((opt) => (
          <button
            key={opt.value}
