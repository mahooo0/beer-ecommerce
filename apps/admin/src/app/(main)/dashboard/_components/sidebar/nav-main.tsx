...
export function NavMain({ items }: NavMainProps) {
  ...
  return (
    <>
      ...
      {items.map((group) => {
        // Template groups are demo / not-yet-working pages: keep them but tuck
        // them behind a dropdown that is collapsed by default.
        const isTemplateGroup = group.label?.startsWith("Template") ?? false;

        if (isTemplateGroup) {
          return (
            <Collapsible key={group.id} defaultOpen={false} className="group/nav-group">
              <SidebarGroup>
                ...
                <CollapsibleContent>
                  <SidebarGroupContent className="flex flex-col gap-2">{renderGroupMenu(group)}</SidebarGroupContent>
                </CollapsibleContent>
              </SidebarGroup>
            </Collapsible>
          );
